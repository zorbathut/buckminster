...
| Same abort, but the type mapping is fine and it's the first call to a pinvoke with a NEW signature *shape* (e.g. the first `double` param ever) | Incremental-build gap: `wasm_m2n_invoke.g.h` regenerates with the new trampoline but the native relink doesn't rerun, so the shipped `dotnet.native.wasm` lacks it (verify: `strings dotnet.native.wasm \| grep -x <cookie>`; the cookie is the return+params letters, e.g. `ILDI` = i32(u64, f64, ptr)) | Delete the host's `obj/**/wasm/for-*` dirs and rebuild (host-wasmnode nests them under `browser-wasm/`, host-web doesn't); only needed when a signature shape is new, not per-export |
