...
# The one agreed native library name (host-as-executor composition): every executor's artifact carries it -- the desktop host crate's cdylib directly, the wasm copy step by rename -- because this constant is baked into shared C# assemblies at compile time.
_LIBRARY = "buckminster"

_CORE_CRATE = "buckminster-core"


@dataclass(frozen=True)
class _CrateInfo:
    """Where one exporting crate's generated C# lands: the owned output directory (components under the repo root) and the namespace its raw/wrapper plumbing lives in."""

    out_dir: tuple[str, ...]
    ffi_namespace: str


# The crate->output mapping, keyed on CARGO_PKG_NAME as the macros record it. An exporting crate absent from this table is a loud GenError, never a silent merge into some other crate's files. Onboarding a crate here also means core adds InternalsVisibleTo for its assembly (the generated plumbing calls core's internal FfiCall/FfiCode). Mirror/trait DEFINITIONS are additionally fenced to core (see _parse_dump) -- generated non-core code references core's mirror types across assemblies through that same IVT.
_CRATES = {
    _CORE_CRATE: _CrateInfo(out_dir=("src", "core", "cs", "Ffi", "Generated"), ffi_namespace="Buckminster.Ffi"),
    "buckminster-host-desktop": _CrateInfo(out_dir=("src", "host-desktop", "cs", "Ffi", "Generated"), ffi_namespace="Buckminster.Host.Desktop.Ffi"),
}
...
def _crate_of(data: dict[str, object], where: str) -> str:
    crate = _expect_str(data, "crate_name", where)
    if crate not in _CRATES:
        raise GenError(f"{where}: crate '{crate}' has no output mapping; teach ffigen's _CRATES table where its generated C# lands before adding exporting crates")
    return crate


# Mirror and trait definitions are fenced to core: a #[buck_struct]/#[buck_enum]/#[buck_trait] in another crate would need a cross-assembly mirror-definition story (which assembly declares the type, who layout-tests it) that stays undesigned until its first consumer forces it. References to core's mirrors from other crates' exports are fine -- that's ordinary cross-assembly type use via InternalsVisibleTo.
def _require_core(crate: str, where: str, what: str) -> None:
    if crate != _CORE_CRATE:
        raise GenError(f"{where}: {what} definitions outside {_CORE_CRATE} have no consumer yet; the cross-assembly mirror-definition design lands with its first consumer")
...
def _parse_dump(dump_path: str) -> tuple[list[Export], list[Struct], list[Enum], list[Trait]]:
    ...
    for data in _expect_list(root, "structs", "dump"):
        ...
        _require_core(_crate_of(data, name), name, "mirror struct")
        fields: list[Field] = []
    ...
    for data in _expect_list(root, "enums", "dump"):
        ...
        _require_core(_crate_of(data, name), name, "mirror enum")
    ...
    for data in _expect_list(root, "traits", "dump"):
        ...
        _require_core(_crate_of(data, name), name, "trait")
        methods: list[Method] = []
...
def _emit_native_methods(crate: str, exports: list[Export], structs: list[Struct], mirrors: _Mirrors) -> str:
    info = _CRATES[crate]
    lines = [_HEADER]
    lines.append("using System.Runtime.InteropServices;")
    if crate != _CORE_CRATE:
        # Non-core plumbing references core's FfiCode (and its internal mirrors) across assemblies -- InternalsVisibleTo from core makes them reachable, this using makes them nameable.
        lines.append("using Buckminster.Ffi;")
    lines.append("")
    lines.append(f"namespace {info.ffi_namespace};")
    lines.append("")
    lines.append("// The generated raw import layer: native snake_case names for 1:1 greppability against the Rust exports, FfiCode returns, out-params unspecified on nonzero return.")
    lines.append("internal static partial class NativeMethods")
    lines.append("{")
    lines.append("    // The one agreed executor-artifact name (host-as-executor composition): every target's native library carries it, whichever crate was the link root.")
    lines.append(f"    private const string Library = \"{_LIBRARY}\";")
...
def _emit_native_wrappers(crate: str, exports: list[Export], mirrors: _Mirrors) -> str:
    info = _CRATES[crate]
    lines = [_HEADER]
    ...
    if crate != _CORE_CRATE:
        # FfiCall and the internal mirrors live in core's plumbing namespace; see _emit_native_methods.
        lines.append("using Buckminster.Ffi;")
    if kinds & {"slice_out", "str"} or crate != _CORE_CRATE:
        lines.append("")
    lines.append(f"namespace {info.ffi_namespace};")
    lines.append("")
...
def generate(dump_path: str, repo_root: str) -> int:
    ...
    try:
        ...
        # Native symbols share one flat namespace in the linked executor regardless of defining crate, so a cross-crate duplicate is a link-time trap the dump must refuse.
        symbols: dict[str, str] = {}
        for export in exports:
            if export.symbol in symbols:
                raise GenError(f"native symbol collision: '{export.symbol}' is exported by both {symbols[export.symbol]} and {export.crate}")
            symbols[export.symbol] = export.crate

        # Wrapper method names only collide within a crate (each crate gets its own Native class in its own assembly).
        for crate in _CRATES:
            methods: dict[str, str] = {}
            for export in exports:
                if export.crate != crate:
                    continue
                method = _pascal(export.name)
                if method in methods:
                    raise GenError(f"wrapper name collision in {crate}: '{methods[method]}' and '{export.name}' both PascalCase to {method}")
                methods[method] = export.name

        # Every mapped crate gets its raw + wrapper files even if it currently has zero exports, so the per-project BuckCheckGenerated guards have a stable file to check. Mirror and trait definitions are core-fenced (_require_core), so those files emit only into core's dir.
        for crate, info in _CRATES.items():
            crate_exports = [export for export in exports if export.crate == crate]
            code_files: dict[str, str] = {}
            _add_file(code_files, "NativeMethods.g.cs", _emit_native_methods(crate, crate_exports, structs if crate == _CORE_CRATE else [], mirrors))
            _add_file(code_files, "Native.g.cs", _emit_native_wrappers(crate, crate_exports, mirrors))
            if crate == _CORE_CRATE:
                for struct in structs:
            ...
            _write_owned_dir(os.path.join(repo_root, *info.out_dir), code_files)
