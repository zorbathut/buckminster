//! The desktop platform layer (PLAN.md M5; hoisted here from core at M5.75 -- platform code is host territory, and the wasm builds now carry no windowing symbols at all): winit wrapped in pump mode behind flat buck_platform_*/buck_window_* exports. Events buffer as plain #[repr(C)] records (core's PlatformEventRaw vocabulary) and C# PULLS them via bulk poll -- no Rust->C# callback ever fires from in here, which is what keeps callback rule 4 trivially honest against winit's own dispatch reentrancy.
//!
...
//!
//! Error-code stance, written down deliberately: unavailability (no display server, dead loop) reports as InvalidArgument with a descriptive message rather than a dedicated code -- per errors-are-bugs, a host that wants to fall back headless PROGRAMMATICALLY needs a capability query (Godot's has_feature analog, future work), not error-code probing.
...
use buckminster_core::ffi::{FfiCode, FfiError, buck_export};
use buckminster_core::keycode::KeyCode;
use buckminster_core::platform::{
    EVENT_KIND_CLOSE_REQUESTED, EVENT_KIND_FOCUS_CHANGED, EVENT_KIND_KEY, EVENT_KIND_RESIZED,
    KEY_FLAG_PRESSED, KEY_FLAG_REPEAT, MODIFIER_ALT, MODIFIER_CONTROL, MODIFIER_META,
    MODIFIER_SHIFT, PlatformEventRaw,
};
use buckminster_core::rid::{Rid, RidAllocator};

// Consumes core's exported single-source keycode list to build the winit translation -- the same list core's enum is built from, so the two can never fork. winit's KeyCode is #[non_exhaustive], so the wildcard arm is mandatory: variants winit adds later degrade to Unidentified rather than failing the build -- accepted, since Unidentified is loud in name and the exhaustive C#<->Rust check still holds.
macro_rules! winit_mapping {
    ($(($name:ident, $value:literal, $winit:ident)),* $(,)?) => {
        fn keycode_from_winit(code: winit::keyboard::KeyCode) -> KeyCode {
            match code {
                $(winit::keyboard::KeyCode::$winit => KeyCode::$name,)*
...
buckminster_core::keycode_list!(winit_mapping);
...
fn window_destroy(window: Rid) -> Result<(), FfiError> {
    with_platform(|state| {
        match state.app.windows.remove(window) {
            Ok(entry) => {
...
fn window_set_title(window: Rid, title: &str) -> Result<(), FfiError> {
    with_platform(|state| match state.app.windows.get(window) {
        Some(entry) => {
...
mod tests {
    ...
    #[test]
    fn winit_mapping_is_name_faithful() {
        // Every winit code we map must land on the identically-named Buckminster code -- spot anchors plus the structural guarantee from core's shared keycode_list.
        assert_eq!(
            keycode_from_winit(winit::keyboard::KeyCode::Escape),
            KeyCode::Escape
        );
        assert_eq!(
            keycode_from_winit(winit::keyboard::KeyCode::KeyW),
            KeyCode::KeyW
        );
        assert_eq!(
            keycode_from_winit(winit::keyboard::KeyCode::ArrowUp),
            KeyCode::ArrowUp
        );
        assert_eq!(
            keycode_from_winit(winit::keyboard::KeyCode::NumpadEnter),
            KeyCode::NumpadEnter
        );
    }
}
