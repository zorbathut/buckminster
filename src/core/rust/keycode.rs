//! Buckminster's raw-key vocabulary: physical (layout-independent) key codes mirroring the W3C UI Events `code` set, which winit's `KeyCode` also mirrors -- so the mapping from winit is name-identical. One macro invocation is the single source of truth for the enum, the winit translation, and the name table; the C# mirror (chunk 2) is verified exhaustively against `buck_test_keycode_name` rather than trusted.
//!
...
// One entry per key: (VariantName, discriminant, WinitVariantName). Generates the enum, name(), from_raw(), COUNT, and the winit mapping -- a single list nothing can drift from. The winit column is explicit because the names are ALMOST all identical by shared W3C ancestry: winit alone calls the OS key Super (SuperLeft/SuperRight) where W3C and we say Meta.
macro_rules! keycodes {
    ($(($name:ident, $value:literal, $winit:ident)),* $(,)?) => {
        ...
        // winit's KeyCode is #[non_exhaustive], so the wildcard arm is mandatory: variants winit adds later degrade to Unidentified rather than failing the build -- accepted, since Unidentified is loud in name and the exhaustive C#<->Rust check still holds.
        #[cfg(all(feature = "window", not(target_os = "emscripten")))]
        pub fn keycode_from_winit(code: winit::keyboard::KeyCode) -> KeyCode {
            match code {
...
mod tests {
    ...
    fn winit_mapping_is_name_faithful() {
        // Every winit code we map must land on the identically-named Buckminster code -- spot anchors plus the structural guarantee from the shared macro list.
        assert_eq!(
